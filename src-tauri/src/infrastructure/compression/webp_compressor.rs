...
impl ImageCompressor for WebpCompressor {
    fn compress(&self, data: &[u8], settings: &CompressionSettings) -> CompressionResult<CompressionOutput> {
        if settings.format != OutputFormat::WebP {
            return Err(CompressionError::UnsupportedFormat("WebpCompressor only supports WebP".to_string()));
        }

        // Charger l'image avec la crate image
        let img = image::load_from_memory(data)
            .map_err(|_e| CompressionError::InvalidImageData)?;
        ...
        let compressed_data = if has_alpha {
            ...
            webp::Encoder::from_rgba(&rgba_data, width, height)
                .encode(quality)
        } else {
            ...
            webp::Encoder::from_rgb(&rgb_data, width, height)
                .encode(quality)
        };
